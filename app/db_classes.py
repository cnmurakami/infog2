...
class Client():
    def __init__(self, id:int = None, name:str = '', email:str = "", cpf:str = ""):
        if cpf != '':
...
class Product():
    def __init__(self, id:int = None, description:str = '', sell_value: float = -1, barcode:str = "", section_id: int = -1, stock: int = -1, expiration_date:str = ""):
        if barcode != '':
    ...
    def get_section_name(self):
        return db_operations.select("SELECT name FROM sections where id = %s limit 1", (self.section_id,), 1)[0]
    ...
    def get_images(self) -> dict:
        ...
        """
        results_raw = db_operations.select(query, (self.id,))
        results = {}
    ...
    def insert_image(self, image:str):
        ...
        result = db_operations.insert("INSERT INTO images(product_id, image) values (%s, %s)", (self.id, bytea_hex), "id")
        return result
...
class Order():
    def __init__ (self, id:int):
        existing_order = db_operations.select("SELECT * FROM orders WHERE id = %s limit 1", (id,), 1)
    ...
    def get_status_description(self, status_id: str):
        try:
            description = db_operations.select("SELECT description from order_status where id = %s LIMIT 1", (status_id,), 1)[0]
            return description
        except:
            raise ObjectNotFound
    ...
    def get_products(self):
        products = {}
        result_raw = db_operations.select("SELECT * FROM orders_products WHERE order_id = %s", (self.id,))
        if result_raw == None or len(result_raw) == 0:
            return products
        for i in range(len(result_raw)):
            product = Product(id = result_raw[i][2])
